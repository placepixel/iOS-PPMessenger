    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (id<PPMessengerContentDisplayGroupProt> group in realArray) {
            ...
            for (NSInteger a = 0; a<[group numberOfItemsInGroup]; a++) {
                ...
                id<PPMessengerConstructionProt> constructor = [self.delegate ppmcm_getConstructorForType:[obj ppm_type]];
                [obj ppm_calculateInBackgroundOnSize:s constructor:constructor withY:dy andIndexPath:[NSIndexPath indexPathForRow:a inSection:[group groupIndex]]];

                if ([constructor ppm_layoutType]==PPMDisplayLayoutTypeHeader) {
                    NSLog(@"header");
                    
                }else{
                    dy+=[obj ppm_layoutAttributes].frame.size.height;
                    dy+=[constructor globalInsets].bottom;
                }
...
//        Class cl = [self.delegate ppmcm_getGroupClassForType:[obj ppm_type]];
