...
#import "PPMOtherUserBubbleConstructor.h"
...
@property (nonatomic,retain)NSMutableDictionary * _groupsClasses;
@end
...
- (void)viewDidLoad {
    ...
    ////
    [self ppm_registerGroupClass:[PPMOtherUserMessagesGroup class] forGroupType:PPMessengerOtherUserMessagesGroup];
    [self ppm_registerGroupClass:[PPMCurrentUserMessagesGroup class] forGroupType:PPMessengerCurrentUserMessagesGroup];
    ///
...
-(NSArray<PPMessengerContentDisplayGroupProt> *)ppml_getAllGroups{
    return [self.contentManager ppm_items];
}
...
-(Class)ppmcm_getGroupClassForType:(NSString*)type{
    
    
   id<PPMessengerConstructionProt> constructor = [self._constructors objectForKey:type];
   Class cl =   [self._groupsClasses valueForKey:[constructor ppm_groupType]];
   return cl;
}
...
-(void)ppm_registerContructor:(id<PPMessengerConstructionProt>)construcor{
    ...
    NSLog(@"%@",[construcor ppm_type]);
    NSLog(@"%@",NSStringFromClass([construcor reusableViewClass]));
    [self.contentView registerClass:[construcor reusableViewClass] forSupplementaryViewOfKind:[construcor ppm_type] withReuseIdentifier:[construcor ppm_type]];
}
-(void)ppm_registerGroupClass:(Class)group forGroupType:(NSString *)groupType{
    [self._groupsClasses setValue:group forKey:groupType];
}

#pragma mark - lazy init
-(NSMutableDictionary *)_groupsClasses{
    if (!__groupsClasses) {
        __groupsClasses=[[NSMutableDictionary alloc] init];
    }
    return __groupsClasses;
